...
class PromptBuilder:
    ...
    def build_daily_from_signals(
        ...
        lines: list[str] = [
            ...
            "",
            "【篇幅与信号比例（必须遵守）】",
            "- 主体分析（今日总览、关键进展清单、趋势与判断）的篇幅**须与下方信号列表中各类型的数量比例一致**。",
            "- 若 type=code/blog/paper/other 等**非视频**信号条数多于 type=video，则上述主体应**主要围绕非视频信号**展开；",
            "  视频仅在第 5 部分「视频精选区」集中呈现，且**主体段落中涉及视频的篇幅不得超过总篇幅的 1/3**。",
            "- 若视频条数明显多于非视频，再适当提高视频相关分析比例。禁止出现「一半以上分析都围绕视频」而忽略大量 GitHub/博客/论文信号的情况。",
            "",
            "【整体结构（请严格按照以下 5 个部分输出，使用 Markdown 二级/三级标题）】",
            "1. 今日总览：用 1~2 段总结今天（或最近数日）最值得关注的 3~5 个主题。",
            "2. 关键进展清单：分为「科研向」与「工程 & 产品向」两小节，列出各 3~7 条重点进展。",
            "3. 趋势与判断：结合近 7/30 天的信号，指出哪些主题在升温/降温，并给出对科研和产品的影响判断。",
            "4. 今日可实践清单：输出 3~5 条明确的、可在 0.5~2 小时内开始执行的行动项（标明 [科研]/[工程]/[产品]）。",
            "5. 视频精选区：从视频类信号中挑选 3~5 条，强调「看完这一条大致能收获什么」。",
            "",
            "【写作要求】",
